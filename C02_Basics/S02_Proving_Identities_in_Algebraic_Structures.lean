...
theorem eq_neg_of_add_eq_zero {a b : R} (h : a + b = 0) : a = -b := by
  rw [← add_zero a, ← add_neg_cancel b]
  rw [← add_assoc]
  rw [h, zero_add]
...
theorem neg_neg (a : R) : - -a = a := by
  rw [← add_zero (- -a), ← neg_add_cancel a]
  rw [← add_assoc]
...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  rw [← one_mul (a * a⁻¹), ← inv_mul_cancel (a⁻¹)]
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  have h1 : a * b * b⁻¹ * a⁻¹ = 1 := by
    rw [mul_assoc a, mul_inv_cancel, mul_one, mul_inv_cancel]
  rw [← mul_one (a * b)⁻¹, ← h1, mul_assoc (a * b), ← mul_assoc (a * b)⁻¹]
